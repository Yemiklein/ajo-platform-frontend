...
import { Users, Wallet, TrendingUp, Clock, ArrowRight, Trophy, Plus } from "lucide-react";
        ...
        <div className="relative rounded-2xl bg-gradient-to-r from-emerald-600 to-emerald-500 px-6 py-5 overflow-hidden">
          ...
          <div className="relative">
            ...
            <p className="text-emerald-100/80 text-sm mt-1">
              {isNewUser
                ? "Welcome! Get started by creating or joining a savings group."
          ...
          {isNewUser && (
            <div className="relative mt-4 flex gap-3">
              <Link
                href="/groups/create"
                className="flex items-center gap-1.5 bg-white text-emerald-700 text-xs font-semibold px-4 py-2 rounded-xl hover:bg-emerald-50 transition-all shadow-sm"
              >
                <Plus size={13} /> Create Group
              </Link>
              <Link
                href="/groups"
                className="flex items-center gap-1.5 bg-white/20 text-white text-xs font-semibold px-4 py-2 rounded-xl hover:bg-white/30 transition-all"
              >
                Join Group
              </Link>
            </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((stat) => {
            ...
            return (
              <div
                key={stat.label}
                className="bg-white dark:bg-zinc-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-zinc-700 relative overflow-hidden hover:shadow-md transition-all duration-200"
              >
            ...
            <div className="p-4">
              {loading ? (
                <div className="space-y-3">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="h-14 bg-gray-50 dark:bg-zinc-700/50 rounded-xl animate-pulse" />
                  ))}
                </div>
              ) : groups.length === 0 ? (
                <div className="text-center py-10">
                  ...
                  <Link
                    href="/groups/create"
                    className="inline-flex items-center gap-1 text-emerald-600 dark:text-emerald-400 text-xs font-medium mt-3 hover:text-emerald-700"
                  >
                    ...
                    <Link href={`/groups/${group.id}`} key={group.id}>
                      <div className="flex items-center justify-between px-3 py-3 rounded-xl hover:bg-gray-50 dark:hover:bg-zinc-700/50 transition-colors cursor-pointer group">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-gray-900 dark:text-white text-sm leading-tight group-hover:text-emerald-700 dark:group-hover:text-emerald-400 transition-colors">
                              {group.name}
                            </p>
                            <p className="text-xs text-gray-400 dark:text-zinc-500 leading-tight mt-0.5">
